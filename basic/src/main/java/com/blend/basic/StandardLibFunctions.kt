...
/**
 * also和let函数功能区别，also也是把接收者作为值参传给Lambda（也就是可以自定义变量），also返回接收者对象，而let返回Lambda结果
 * also和apply很相似，都是返回接收者对象，区别就是also能自定义变量，但是apply不能
 *
 * also尤其适合针对同一原始对象，在类里面能防止竞态条件。
 *
 * 静态条件：对于一个即可空又可变的属性，虽然在使用它之前做了非空判断，但是编译器还是不通过。原因是如果应用程序里有其他代码同时修改
 * 某个代码状态，并产生了难以预料的结果，这就出现了竞态条件。
 */
fun alsoFunc() {
